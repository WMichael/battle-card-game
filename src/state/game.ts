...
export function drawCards(state: GameState): GameState {
    ...
    if (playerCard1 && playerCard2) {
      state.players.player1.hand.push(playerCard1);
      state.players.player2.hand.push(playerCard2);
    }
    return state;
  }
...
export function awardCards(state: GameState): GameState {
  ...
  const playerCard1 = state.players.player1.hand.pop();
  const playerCard2 = state.players.player2.hand.pop();
  if (playerCard1 && playerCard2) {
...
export function compareCards(state: GameState): Player {
  const player1Card = state.players.player1.hand[0];
  const player2Card = state.players.player2.hand[0];
